...
插件初版介绍：https://youtu.be/Zz-B8ijCpZM

接坑预览介绍：https://www.bilibili.com/video/BV1AM411V7i3/
...
- [ ] 游戏基本内容
	- [ ] 游戏流程指引
	- [ ] 单人战局（其他队伍用AI来代替，AI只会建筑升级，不会主动出击 另外设计一个小型地图）
	- [x] 建筑被攻击/拆除提示
	- [x] 基本游戏流程的实现
	- [x] 炮塔防御类建筑的实现
	- [x] 实现预设三种AI的全部效果
	...
	- [x] 开始游戏时给初始资源，钻石镐菜单换成其他物品
	- [x] 矿物资源的分布设置，选定点随机生成，每个队伍周围矿物均等（生成概率100%）
	- [ ] 地图大小过大，需要和小地图配合，或者更改地图道路指引
	- [x] 地图道路更改，更改为连接队伍的核心，以便寻找 *（虾壳）*
	- [ ] 地图增加随机建筑机制
	- [ ] 玩家参与游戏攻击方式的设计实现（远程攻击）
	- [x] 维修机制：维修工程师/光陵修复塔，塔范围内的建筑自动回血，每X秒自动回一点血
	- [x] 防御炮塔需要消耗资源才能正常运行
	- [ ] 增加消耗能量的飞行背包(鞘翅+烟花)
	- [x] 避免刷屏：奖励积分提示改为actionbar/悬浮字，防御塔补弹提示更改
	- [ ] 玩家回城选项机制，需要CD
	- [ ] 陷阱建筑（蜘蛛网等）
	- [x] 允许后进玩家加入游戏
	- [ ] 建筑攻击提示间隔
	...
	- [ ] 建筑有效范围显示
	- [x] NPC攻击玩家不掉血
	- [x] 玩家初始物品存放方式更改（是否设置为死亡不掉落，铁镐耐久度等）
	- [ ] 扩展游戏内容玩法：粘液科技（zim以前在PVE实现的NPC，防御塔等）
	- [ ] 基地被拆后所有建筑报废
	- [ ] 积分面板更改显示其他队伍的团队积分
	- [ ] 小队名字颜色区分，小队装备（染色皮革套）区分
	- [ ] 玩家死亡掉落资源，死亡复活CD设置
	- [ ] 防御塔强度升级更改：一级伤害低，无击退，攻击速率低，消耗资源少；二级伤害中等，击退低，攻击速率中等，消耗资源中等；二级伤害高，击退高，攻击速率高，消耗资源大
	- [ ] 铁匠铺机制更改：铁匠铺NPC从主城获取资源随机造装备，装备玩家，NPC可用，可制造鞘翅，烟花等装备（喷气背包）
	- [ ] 主城自带防御
	- [ ] 队伍数量可以根据配置文件调整
	- [x] 游戏结算
	- [ ] 制作成完整的RTS小游戏 [80%]
...
- [ ] 附加内容
	- [ ] 建筑皮肤
	- [ ] 安排插件玩法官网，实现放资源（地图模板，建筑模板），可以考虑使用论坛程序
